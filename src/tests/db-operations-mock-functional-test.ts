/**
 * Functional Tests for Database Operations Using Mocks
 * 
 * This test file conducts end-to-end testing of the database operations,
 * creating mock data, performing CRUD operations, and cleaning up afterward.
 * It uses in-memory mocks instead of actual Supabase connections.
 * 
 * To run this file:
 * npx tsx src/tests/db-operations-mock-functional-test.ts
 */
...
async function testTherapistOperations() {
  ...
  try {
    // CREATE THERAPIST
    const therapistData = {
      user_id: `${TEST_PREFIX}-user-id`,
      full_name: 'Test Therapist',
      credentials: 'PhD, Testing',
      specialties: ['Unit Testing', 'Integration Testing'],
      bio: 'Professional tester with years of experience'
    };
    ...
    logSuccess(`Retrieved all therapists (${getAllResult.data!.length} found)`, getAllResult.data);
...
async function runFunctionalTests() {
  console.log('🔍 Starting PsyPlex DB Operations Mock Functional Tests');
  console.log('================================================');
  ...
  if (therapistTestsPassed) {
    ...
    if (clientTestsPassed) {
      // These tests can run in parallel as they only depend on client and therapist
